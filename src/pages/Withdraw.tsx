...
const Withdraw = () => {
  ...
  // Fetch withdraw methods from database
  const { data: withdrawMethods, error, isLoading } = useQuery({
  ...
  React.useEffect(() => {
    if (error) {
      notify.error('Failed to load withdraw methods');
    }
  }, [error]);

  const handleWithdrawMethodClick = (method: any) => {
    // Navigate to withdraw detail page with method ID as route parameter
    navigate(`/withdraw/${method.id}`);
  };
            ...
            {isLoading ? (
              ...
              >
                ...
                <div className="flex-auto">
                  ...
                  <h1 className="_text-[10px]_lq8ol_131 _text-gray-500_lq8ol_575">Withdraw with currency: {method.currency}</h1>
                </div>
                <div className="mt-1">
